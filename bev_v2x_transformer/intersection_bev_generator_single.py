...
def ge_single_label_from_whole(cursor, veh, timestamp, table, area_range, single_range, mask_ori_label, ratio):
    ...
    return single_ori_label, ego_label, para_list
...
if __name__ == '__main__':
    ...
    for k in range(0, 8):
        ...
        for index in range(69, len(timestamp_list) - 10, 10):
            ...
            npy_pre = np.zeros((13, 16, 6, int(single_range / ratio), int(single_range / ratio)), np.float)
            mask_pre = np.zeros((13, 16), np.uint8)
            para_pre = np.zeros((13, 16, 7), np.float)
            ego_pre = np.zeros((13, 16, int(single_range / ratio), int(single_range / ratio)), np.uint8)
            if not os.path.exists("./BEVImgData_01_4_single_pro/" + str(k) + "/" + str(timestamp_now)):
                os.makedirs("./BEVImgData_01_4_single_pro/" + str(k) + "/" + str(timestamp_now))
            if not os.path.exists("./BEVData_01_4_single_pro"):
                os.makedirs("./BEVData_01_4_single_pro/")
            # if not os.path.exists("./BEVImgLabel_01_4_single/" + str(k) + "/" + str(timestamp_now)):
            #     os.makedirs("./BEVImgLabel_01_4_single/" + str(k) + "/" + str(timestamp_now))
            if not os.path.exists("./BEVMaskData_01_4_single_pro"):
                os.makedirs("./BEVMaskData_01_4_single_pro")
            if not os.path.exists("./BEVParaData_01_4_single_pro"):
                os.makedirs("./BEVParaData_01_4_single_pro")
            if not os.path.exists("./BEVEgoData_01_4_single_pro"):
                os.makedirs("./BEVEgoData_01_4_single_pro")
            # if not os.path.exists("./BEVLabel_01_4_single"):
            #     os.makedirs("./BEVLabel_01_4_single")
            for time_idx in range(13):
                for veh_idx in range(len(vehicle_fu_list)):
                    ...
                    print(timestamp, veh)
                    local_x, local_y, orien = utils.SearchTrafficParticipantTiming(cursor, veh, timestamp, table)
                    single_ori_label, ego_label, para_list = ge_single_label_from_whole(cursor, veh, timestamp,
                    ...
                    para_list.append(orien)
                    para_list = np.array(para_list).astype(np.float)
                    mask_pre[time_idx][veh_idx] = 1
                    para_pre[time_idx][veh_idx] = para_list
                    ego_pre[time_idx][veh_idx] = ego_label
                    cv2.imwrite("./BEVImgData_01_4_single_pro/" + str(k) + "/" + str(timestamp_now) + "/" +
                                str(timestamp) + "_" + str(veh) + ".png", mask_fig[:, :, ::-1])

                    # cv2.imshow("display", label_fig[:, :, ::-1])
                    # cv2.waitKey(0)

            # cv2.imwrite("./BEVImgLabel_01_6/" + str(k) + "/" + str(timestamp_now) + "/" +
            ...
            np.save("./BEVData_01_4_single_pro/" + str(k) + "_" + str(timestamp_now) + ".npy", npy_pre)
            np.save("./BEVMaskData_01_4_single_pro/" + str(k) + "_" + str(timestamp_now) + ".npy", mask_pre)
            np.save("./BEVParaData_01_4_single_pro/" + str(k) + "_" + str(timestamp_now) + ".npy", para_pre)
            np.save("./BEVEgoData_01_4_single_pro/" + str(k) + "_" + str(timestamp_now) + ".npy", ego_pre)
            # np.save("./BEVLabel_01_4_single/" + str(k) + "_" + str(timestamp_now) + ".npy", mask_ori_label)
