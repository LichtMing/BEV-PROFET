...
def add_ego_vehicles_to_scenario(
    scenario: Scenario, planning_problem_set: PlanningProblemSet, vehicle_params, active_learning=False
):
    ...
    for (
        ...
        if active_learning:
            agent_id = planning_problem_id - 70000
    ...
    def plan_global_path(self, scenario, planning_problem, vehicle_params, initial_state=None):
        ...
        with self.exec_timer.time_with_cm("initialization/plan global path"):
            ...
            # raise error if global path planner fails
                ...
                state_list = self.scenario.obstacle_by_id(self.ego_id).prediction.trajectory.state_list
                self.global_path_length = 20 + min(len(state_list) // 10, 40)
                selected_indices = np.linspace(0, len(state_list) - 1, self.global_path_length, dtype=int)
                initial_global_path = []
                for indice in selected_indices:
                    initial_global_path.append(state_list[indice].position)
                initial_global_path = np.array(initial_global_path)
                print("Could not find a global path for the planning problem. Use sampling trajectory instead. ")
        ...
        with self.exec_timer.time_with_cm("initialization/check curvature of path"):
            ...
            global_path_new = [self.global_path[0]]
            for i in range(1, len(self.global_path)):
                if abs(self.global_path[i, 0] - global_path_new[-1][0]) < 0.001 and \
                    abs(self.global_path[i, 1] - global_path_new[-1][1]) < 0.001:
                    pass
                else:
                    global_path_new.append(self.global_path[i, :])
            self.global_path = np.asarray(global_path_new)
