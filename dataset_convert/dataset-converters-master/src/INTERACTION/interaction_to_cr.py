...
def generate_single_scenario(output_dir, id_segment, tags, interaction_config, dt: float, scenario_time_steps: int,
    ...
    if num_planning_problems == 0:
        # Convert ALL eligible CAR obstacles to planning problems
        car_obstacles = [obs for obs in scenario.dynamic_obstacles
                         if obs.obstacle_type == ObstacleType.CAR
                         and obs.initial_state.time_step == 0]
        if len(car_obstacles) == 0:
            return

        # Compute max_time_step from ALL obstacles before removing any
        max_time_step = max(
        ...
        for obs in car_obstacles:
            final_time_step = min(
                obs.prediction.trajectory.final_state.time_step + 25,
                max_time_step
            )
            pp = obstacle_to_planning_problem(
                obs, planning_problem_id=obs.obstacle_id,
                final_time_step=final_time_step
            )
            planning_problem_set.add_planning_problem(pp)

        # Remove all converted cars from scenario
        for obs in car_obstacles:
            scenario.remove_obstacle(obs)
    else:
        # Original behavior: random selection of N vehicles
...
def generate_scenarios_for_map(location: str, map_dir: str, input_dir: str, output_dir: str, interaction_config,
    ...
    path_files = sorted(glob.glob(os.path.join(directory_data, "vehicle_tracks_*.csv")))
