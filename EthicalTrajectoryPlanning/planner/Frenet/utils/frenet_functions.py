...
from planner.Frenet.utils.validity_checks import VALIDITY_LEVELS, check_validity, check_validity_old
...
class FrenetTrajectory:
    ...
    def __init__(
        ...
        target_d: float = 0,
        target_v: float = 0,
        target_behavior: (str, str) = ("Keep", "Cruise")
    ):
        """
    ...
    def get_global_state(self, item):
        return State(position=[self.x[item], self.y[item]],
                     orientation=self.yaw[item],
                     velocity=self.v[item],
                     )

    def get_frenet_state(self, item):
        return [self.s[item], self.s_d[item], self.s_dd[item],
                self.d[item], self.d_d[item], self.d_dd[item]]
...
def calc_frenet_trajectories_combination_based(
    c_s: float,
    c_s_d: float,
    c_s_dd: float,
    c_d: float,
    ...
    fp_list = []
    # all end velocities
    for behavior, (dT, vT) in zip(behavior_combination, d_v_combination):
        if abs(c_s_d) < v_thr or abs(vT) < v_thr:
            lat_mode = "low_velocity"
        else:
            lat_mode = "high_velocity"
...
def sort_frenet_trajectories(
    ...
    for fp in fp_list:
        with timer.time_with_cm("simulation/sort trajectories/check validity/total"):
            # check validity
            fp.valid_level, fp.reason_invalid = check_validity_old(
                ft=fp,
                ego_state=ego_state,
                scenario=scenario,
                vehicle_params=vehicle_params,
...
def get_v_list(
    ...
    if v_goal_min is None:
        ...
        v_append_list = [v_cur, v_min, v_max, v_goal]

    if abs(v_cur - v_goal) < 1:
        v_append_list.remove(v_goal)
        n_samples = 3
    if abs(v_cur - v_min) < 1 or abs(v_cur - v_max) < 1:
        v_append_list.remove(v_cur)
        n_samples = min(n_samples - 1, 3)
    v_list = []
