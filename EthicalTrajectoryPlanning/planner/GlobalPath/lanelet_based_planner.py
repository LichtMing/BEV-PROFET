        ...
        if (
            ...
            if hasattr(self.planningProblem.goal.state_list[0], 'position'):
                if hasattr(self.planningProblem.goal.state_list[0].position, 'center'):
                    goal_center = self.planningProblem.goal.state_list[0].position.center
                    # Handle 0-d array wrapping a shapely Point
                    goal_center = np.asarray(goal_center)
                    if goal_center.ndim == 0:
                        point_obj = goal_center.item()
                        goal_center = np.array([point_obj.x, point_obj.y])
                    self.goalLanelet_ids = (
