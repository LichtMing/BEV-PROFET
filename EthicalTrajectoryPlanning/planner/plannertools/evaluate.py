        ...
        with self.exec_timer.time_with_cm("total"):
            try:
                ...
                if "Goal reached but time exceeded!" in str(excp):
                    ...
                    # write new scenario
                    fw = CommonRoadFileWriter(self.scenario, PlanningProblemSet(), author, affiliation, source, tags)
                ...
                # import traceback
                # traceback.print_exc()
                print(f"{scenario_path} >>> {str(traceback.print_exc())}")
                return_dict = {"success": False, "reason_for_failure": str(excp)}
