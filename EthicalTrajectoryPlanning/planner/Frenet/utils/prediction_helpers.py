...
def collision_checker_prediction(
    predictions: dict, scenario, ego_co, frenet_traj, ego_state, anchor_time,
):
    """
    ...
    for obstacle_id in list(predictions.keys()):
        ...
        if not hasattr(scenario.obstacle_by_id(obstacle_id).obstacle_shape, 'length'):
            ...
            pred_traj = predictions[obstacle_id]['pos_list']
            pred_length = min(int(frenet_traj.t[-1] / 0.1), len(pred_traj))
            if pred_length == 0:
                continue



            time_interval = int((frenet_traj.t[1] - frenet_traj.t[0]) / 0.1)
            start_time_step = ego_state.time_step - anchor_time + time_interval - 1
            end_time_step = start_time_step + pred_length
            # clamp to prediction array length to avoid OOB slicing
            end_time_step = min(end_time_step, len(pred_traj))
            if end_time_step <= start_time_step:
                continue
            # get x, y and orientation of the prediction
            x = pred_traj[:, 0][start_time_step:end_time_step:time_interval]
            y = pred_traj[:, 1][start_time_step:end_time_step:time_interval]
            pred_orientation = predictions[obstacle_id]['orientation_list'][start_time_step:end_time_step:time_interval]
            traj_length = len(x)  # use actual sliced length, not formula
            if traj_length == 0:
                continue
            # create a time variant collision object for the predicted vehicle
            traj = [[x[i], y[i], pred_orientation[i]] for i in range(traj_length)]
            ...
            if collision_at[0] != -1:
                collision_cov_list = predictions[obstacle_id]['cov_list'][start_time_step:end_time_step, :]
                return True, collision_at, collision_cov_list
