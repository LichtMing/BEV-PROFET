...
from planner.Frenet.utils.prediction_helpers import (
    collision_checker_prediction, get_obstacles_in_radius,
)
...
def check_validity(
    ...
    # check maximum velocity
    if not velocity_valid(ft, vehicle_params):
        return 0, "velocity", -1, None

    #
    # # check maximum acceleration:
    if not acceleration_valid(ft, vehicle_params):
        return 0, "acceleration", -1, None
    ...
    if reason_curvature_invalid is not None:
        return 0, f"curvature ({reason_curvature_invalid})", -1, None
    ...
    # check road boundaries
    # if ft.bd_harm is calculated, use it for boundary check
    if ft.target_behavior[0] != "Keep":
        leaving, leaving_time = boundary_valid(vehicle_params, collision_object, road_boundary)
        if not leaving:
            return 2, "boundaries", leaving_time - ego_state.time_step, None



    collision_detected, collision_at, collision_cov_list = collision_checker_prediction(
        predictions=predictions,
        scenario=scenario,
        ego_co=collision_object,
        frenet_traj=ft,
        ego_state=ego_state,
        anchor_time=anchor_state.time_step,
    )
    if collision_detected:
        return 1, "collision", collision_at[0] - ego_state.time_step, collision_cov_list

    # obj_ids, dynamic_obstacle_objects = create_dynamic_obstacle_objects(scenario=scenario, initial_step=ego_state.time_step,
    #                                                            end_step=ego_state.time_step + check_horizon,
    #                                                            ego_id=ego_id,
    #                                                            anchor_state=anchor_sense_state if anchor_sense_state is not None else ego_state)
    # if len(dynamic_obstacle_objects) > 0:
    #     result = trajectory_queries.trajectories_collision_dynamic_obstacles(trajectories=[collision_object],
    #                                                                          dynamic_obstacles=dynamic_obstacle_objects,
    #                                                                          method='grid',
    #                                                                          num_cells=32,
    #                                                                          )
    #
    #
        # if result[0] != -1:
        #     return 1, "collision", result[0] - ego_state.time_step
        #


    # if not max_risk_valid(ft, risk_params, mode):
    #     return 3, "max_risk", -1, None
...
def create_collision_object(ft, vehicle_params, ego_state):
    ...
    #     traj_list = [[ft.x[i], ft.y[i], ft.yaw[i]] for i in scope]
    # else:
...
def create_dynamic_obstacle_objects(scenario: Scenario, initial_step, end_step, ego_id, anchor_state, sensor_radius=50):
    """ create collision objects in the scenario
    ...
    visible_obstacles = get_obstacles_in_radius(
        scenario=scenario,
        ego_id=ego_id,
        ego_state=anchor_state,
        radius=sensor_radius,
    )

    for obs in scenario.dynamic_obstacles:
        if obs.obstacle_id == ego_id or obs.obstacle_id not in visible_obstacles:
            continue
        elif end_step < obs.initial_state.time_step or initial_step > obs.prediction.final_time_step:
            continue
        else:
            if initial_step <= obs.initial_state.time_step:
                traj = [[obs.initial_state.position[0], obs.initial_state.position[1], obs.initial_state.orientation]]
            else:
                traj = []

            ini = max(obs.prediction.initial_time_step, initial_step)
            end = min(obs.prediction.final_time_step, end_step)
            for s in obs.prediction.trajectory.states_in_time_interval(ini, end):
                traj.append([s.position[0], s.position[1], s.orientation])

            collision_object_raw = create_tvobstacle(
                            traj_list=traj,
                            box_length=obs.obstacle_shape.length / 2,
                            box_width=obs.obstacle_shape.width / 2,
                            start_time_step=max(initial_step, obs.initial_state.time_step),
                        )
            ...
            if err:
                collision_object = collision_object_raw
            objects_id.append(obs.obstacle_id)
            collision_objects.append(collision_object)

    return objects_id, collision_objects
...
def boundary_valid(vehicle_params, collision_object, road_boundary):
    ...
    if leaving_road_at[0] != -1:
        return False, leaving_road_at[0]

    return True, leaving_road_at[0]
